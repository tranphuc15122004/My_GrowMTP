...
class SGLangHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if self.config.mtp is not None and self.config.mtp.enable and self.config.mtp.enable_rollout:
            ...
            if self.config.mtp.growmtp:
                args.update(growmtp_collect_signals=True,
                            speculative_classic_rejection_sampling=True,
                            growmtp_teacher_topk=self.config.mtp.teacher_topk,
    ...
    async def generate(
        ...
        mtp_signals = meta_info.get("mtp_signals")
        if mtp_signals:
            extra_fields["mtp_signals"] = mtp_signals
        ...
        if self.config.mtp is not None and self.config.mtp.enable and self.config.mtp.enable_rollout:
            extra_fields["spec_num_draft_tokens"] = int(meta_info.get("spec_num_proposed_drafts", 0))
            ...
            if "spec_correct_drafts_histogram" in meta_info:
                extra_fields["spec_correct_drafts_histogram"] = meta_info["spec_correct_drafts_histogram"]
