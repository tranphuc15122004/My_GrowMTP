...
class FSDPEngine(BaseEngine):
    ...
    def _build_optimizer(self, module):
        ...
        if self.model_config.mtp.growmtp:
            from verl.trainer.mtp.engine import optimizer_groups
            params = optimizer_groups(module, self.model_config.mtp)
    ...
    def _build_lr_scheduler(self, optimizer):
        ...
        if self.model_config.mtp.growmtp:
            ...
            from verl.trainer.mtp.engine import head_schedule
            mtp = self.model_config.mtp
            lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, [
                lr_scheduler.lr_lambdas[0],
                partial(head_schedule, total_steps=total_steps, warmup_steps=mtp.warmup_steps,
                        min_ratio=mtp.min_lr_ratio),
            ])
        return lr_scheduler
    ...
    def _build_model_optimizer(self):
        ...
        # Fused vocabulary projection reads lm_head.weight without its forward hook.
        # Keep shared layers in the root FSDP group on actor AND reference engines.
        module._fsdp_shared_layers_in_root = self.model_config.use_fused_kernels
        ...
        mtp = self.model_config.mtp
        if mtp.growmtp and not self.engine_config.forward_only:
            if self.engine_config.strategy != "fsdp2" or self.ulysses_sequence_parallel_size != 1:
                raise ValueError("GrowMTP requires FSDP2 with sequence_parallel_size=1")
            if not mtp.enable_train or not mtp.enable_rollout or not mtp.detach_encoder:
                raise ValueError("GrowMTP requires train/rollout enabled and detached encoder")
            from verl.models.transformers.mtp import attach_mtp
            module = attach_mtp(module, self.model_config.local_path)
