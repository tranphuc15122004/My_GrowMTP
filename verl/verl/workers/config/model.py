...
# MiMo backbone IS Qwen2; its config.json has auto_map so AutoConfig.from_pretrained
# (trust_remote_code=False, in HFModelConfig.__post_init__) would raise. Register thin
# Qwen2 subclasses under model_type='mimo' at import time so it loads as a native Qwen2
# (no remote code), all FSDP hooks apply, model_type stays 'mimo' for the MTP head factory +
# sglang draft auto-select. Module-level (not local) so the loaded config pickles to the
# sglang scheduler subprocesses. mtp_layers.* are unexpected-keys (loaded by inject).
try:
    from transformers import AutoConfig as _opd_ac, AutoModelForCausalLM as _opd_aclm
    from transformers.models.qwen2.configuration_qwen2 import Qwen2Config as _opd_q2c
    from transformers.models.qwen2.modeling_qwen2 import Qwen2ForCausalLM as _opd_q2m

    class MiMoConfig(_opd_q2c):
        model_type = "mimo"

    class MiMoForCausalLM(_opd_q2m):
        config_class = MiMoConfig

    _opd_ac.register("mimo", MiMoConfig, exist_ok=True)
    _opd_aclm.register(MiMoConfig, MiMoForCausalLM, exist_ok=True)
except Exception as _opd_e:
    print(f"[GrowMTP] MiMo native registration skipped: {_opd_e}")
