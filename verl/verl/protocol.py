...
class DataProto:
    ...
    def slice(self, start=None, end=None, step=None):
        ...
        if self.batch is not None:
            # GrowMTP nested-aware slice: TensorDict slicing (self.batch[slice_obj])
            # raises on jagged NestedTensors (mtp_* signal fields). Slice nested keys
            # via unbind -> list slice -> rebuild; regular keys via TensorDict slicing.
            _nested_keys = {
                k for k, v in self.batch.items() if isinstance(v, torch.Tensor) and v.is_nested
            }
            if _nested_keys:
                from verl.utils.tensordict_utils import nested_tensor_from_tensor_list

                _regular = TensorDict(
                    {k: v for k, v in self.batch.items() if k not in _nested_keys},
                    batch_size=self.batch.batch_size,
                    device=self.batch.device,
                )
                sliced_batch = _regular[slice_obj]
                _sel = list(range(*slice_obj.indices(self.batch.batch_size[0])))
                for _key in _nested_keys:
                    _nt = self.batch[_key]
                    _ragged_idx = getattr(_nt, "_ragged_idx", _nt.dim() - 1)
                    try:
                        _rows = list(_nt.unbind(dim=0))
                        _picked = [_rows[i] for i in _sel]
                    except RuntimeError:
                        # 3D+ jagged unbind bug (pytorch#153238): fall back to padded.
                        _padded = _nt.to_padded_tensor(0)
                        _lengths = _nt.offsets().diff().tolist()
                        _picked = [_padded[i][: _lengths[i]] for i in _sel]
                    sliced_batch[_key] = nested_tensor_from_tensor_list(_picked, ragged_idx=_ragged_idx)
            else:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if batch_lst[0] is not None:
            # GrowMTP nested-aware concat: delegate to verl's concat_tensordict so jagged
            # nested tensors (mtp_* signal fields) concat via concat_nested_tensors instead
            # of plain torch.cat (which raises on NestedTensor dim=0). Equivalent to
            # torch.cat for the all-regular case.
            from verl.utils.tensordict_utils import concat_tensordict

            new_batch = concat_tensordict(batch_lst)
        else:
            new_batch = None
    ...
    def reorder(self, indices):
        ...
        self.batch = index_select_tensor_dict(self.batch, indices)
