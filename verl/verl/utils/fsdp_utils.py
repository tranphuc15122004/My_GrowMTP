...
def _select_fsdp2_wrap_targets(model, fsdp_transformer_layer_cls_to_wrap):
    ...
    has_growmtp = mtp is not None and mtp.__class__.__name__ == "MTPHead"
    _tie = getattr(model.config, "tie_word_embeddings", False) or has_growmtp or getattr(model, "_fsdp_shared_layers_in_root", False)
    _wrap_by_name = set() if _tie else {"embed_tokens", "lm_head"}

    modules = []
    for name, module in model.named_modules():
        if has_growmtp and name.startswith("mtp."):
            continue
        if has_growmtp and name == "mtp":
