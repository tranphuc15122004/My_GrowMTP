...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            copy_processor_assets(getattr(model_config, "_name_or_path", ""), hf_config_tokenizer_path)
            log_with_rank(
            ...
            # loaded from the Hub.
            if hasattr(model_config, "auto_map") and model_config.auto_map:
                # unwrap_model is a native Qwen2 subclass (MiMo loaded via local
                # registration): _auto_class is None and fully_shard renamed its __class__
                # to a torch fsdp module. custom_object_save(unwrap_model, ...) would set
                # auto_map[None]=... (-> to_json_string sort_keys crash on None<str) and copy
                # torch _fsdp_*.py instead of modeling_mimo.py. Keep the genuine auto_map and
                # copy the real source files from the original model dir.
                import shutil as _opd_shutil
                model_config.auto_map = {
                    _k: _v for _k, _v in model_config.auto_map.items() if _k is not None
                }
                _opd_src = getattr(model_config, "_name_or_path", "") or getattr(
                    model_config, "name_or_path", ""
                )
                if _opd_src and os.path.isdir(_opd_src):
                    _opd_files = set()
                    for _v in model_config.auto_map.values():
                        for _e in ((_v,) if isinstance(_v, str) else _v):
                            if _e:
                                _opd_files.add(_e.split(".")[0] + ".py")
                    for _fn in _opd_files:
                        _src_f = os.path.join(_opd_src, _fn)
                        if os.path.isfile(_src_f):
                            _opd_shutil.copy(
                                _src_f, os.path.join(hf_config_tokenizer_path, _fn)
                            )
                    model_config.save_pretrained(hf_config_tokenizer_path)
                else:
                    custom_object_save(
                        unwrap_model, hf_config_tokenizer_path, config=model_config
                    )
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                if isinstance(getattr(unwrap_model, "mtp", None), MTPHead):
                    # Keep resumable .pt state in master precision; the HF serving export
                    # uses the same BF16 dtype as rollout (including hybrid-state kernels).
                    state_dict = {key: value.to(torch.bfloat16) if value.is_floating_point() else value
                                  for key, value in state_dict.items()}
                    save_model.to(dtype=torch.bfloat16)
                    save_model.config.dtype = torch.bfloat16
                    if hasattr(save_model.config, "text_config"):
                        save_model.config.text_config.dtype = torch.bfloat16
...
def copy_processor_assets(source, destination):
    """Keep text-use exports of multimodal checkpoints independently loadable by SGLang."""
    from pathlib import Path
    import shutil
    if not source:
        return
    for name in ("preprocessor_config.json", "video_preprocessor_config.json", "processor_config.json"):
        origin = Path(source) / name
        target = Path(destination) / name
        if origin.is_file() and origin.resolve() != target.resolve():
