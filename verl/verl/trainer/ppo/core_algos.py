...
@register_policy_loss("asym")  # type: ignore[arg-type]
def compute_policy_loss_asym(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    """GrowMTP asymmetric GRPO loss: down-weight negative-advantage updates by w_neg.

    Identical to compute_policy_loss_vanilla (same dual-clip, same agg, same metrics)
    EXCEPT per-token loss for tokens with advantage < 0 is scaled by
    w_neg = negative_advantage_weight (default 1.0 == vanilla). w_neg<1 reduces the
    over-penalization of wrong responses in the high pass@K regime, protecting
    shared prefixes / correct intermediate steps / answer format from being
    eroded by the symmetric negative gradient. Preserves token-mean (no length bias).
    """
    import os as _os
    assert config is not None
    assert not isinstance(config, AlgoConfig)
    w_neg = config.policy_loss.negative_advantage_weight
    ...
    # ONLY change vs vanilla: scale the negative-advantage branch by w_neg.
    pg_losses = torch.where(advantages < 0, w_neg * clip_pg_losses2, clip_pg_losses1)
