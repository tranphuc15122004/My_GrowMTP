...
class RayPPOTrainer:
    ...
    def _compute_ref_log_prob(self, batch: DataProto) -> DataProto:
        from verl.trainer.mtp.signals import logprob_inputs

        batch = logprob_inputs(batch)
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if spec_histograms is not None:
                    ...
                    if _valid_hists:
                        _k = max(len(h) - 1 for h in _valid_hists)
                        # micro (pooled) aggregation across requests
                        _num = [0 for _ in range(_k)]  # sum S_{j+1}: cycles accepting > j drafts
                        _den = [0 for _ in range(_k)]  # sum S_j:     cycles reaching step j
                        for _h in _valid_hists:
                            _total = sum(_h)
                            if _total == 0:
                                continue
                            _cumsum = _total
                            for _j in range(min(_k, len(_h) - 1)):
                                _reached = _cumsum
                                _cumsum -= _h[_j]
                                _den[_j] += _reached
                                _num[_j] += _cumsum
                        for _si in range(_k):
                            if _den[_si] > 0:
                                metrics[f"rollout/mtp/acc_rate_step{_si}"] = _num[_si] / _den[_si]
                        # overall_acc_rate: same method as rollout/spec_accept_rate
                        # (per-request accepted/proposed, macro-averaged over requests)
                        _per_req = []
                        for _h in _valid_hists:
                            _total = sum(_h)
                            _proposed = (len(_h) - 1) * _total
                            if _proposed > 0:
                                _accepted = sum(_j * _h[_j] for _j in range(len(_h)))
                                _per_req.append(_accepted / _proposed)
                        if _per_req:
