...
class Engine(EngineScoreMixin, EngineBase):
    ...
    def update_weights_from_tensor(
        ...
        if load_format == "flattened_bucket":
            ...
            # Tensor data → named /dev/shm via ctypes.memmove (~0.3s for 462MB).
            # Only tiny metadata dict goes through IPC. No torch_shm_manager spawned,
            # no resource_sharer fd conflicts, multi-consumer safe for dp>1. ===
            ...
            _total_bytes = 0
            _cpu_tensors = []
            for _name, _tensor in named_tensors:
                _t = _tensor.contiguous()
                if not _t.is_cpu:
                    _t = _t.cpu()
                _nbytes = _t.untyped_storage().nbytes()
                _meta_list.append((_name, tuple(_t.shape), str(_t.dtype), _total_bytes, _nbytes))
                _cpu_tensors.append(_t)
                _total_bytes += _nbytes
            _shm_name = f"growmtp_{_uuid.uuid4().hex[:16]}"
            _shm = _SharedMemory(name=_shm_name, create=True, size=max(_total_bytes, 1))
            for _t, (_, _, _, _offset, _nbytes) in zip(_cpu_tensors, _meta_list):
                _ctypes.memmove(
                    _ctypes.addressof(_ctypes.c_char.from_buffer(_shm.buf, _offset)),
                    _t.data_ptr(), _nbytes)
            serialized_named_tensors = [{"_opd_shm": True, "shm_name": _shm_name,
                                         "total_bytes": _total_bytes, "metadata": _meta_list}]
        obj = UpdateWeightsFromTensorReqInput(
            serialized_named_tensors=serialized_named_tensors,
            load_format=load_format,
            flush_cache=flush_cache,
        )
        _result = self.loop.run_until_complete(
            self.tokenizer_manager.update_weights_from_tensor(obj, None)
        )
        # Cleanup: unlink shm after all workers finished reading
        _shm.close()
        _shm.unlink()
        return _result
