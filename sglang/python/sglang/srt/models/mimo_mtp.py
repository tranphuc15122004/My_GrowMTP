        ...
        for name, loaded_weight in weights:
            if name.startswith("mtp."):
                if name == "mtp.fc.weight":
                    # un-swap [embed,hidden] -> native [hidden,embed] input_proj
                    _H = loaded_weight.shape[0]
                    loaded_weight = torch.cat(
                        [loaded_weight[:, _H:], loaded_weight[:, :_H]], dim=-1
                    ).contiguous()
                    name = "model.mtp_layers.0.input_proj.weight"
                elif name == "mtp.pre_fc_norm_embedding.weight":
                    name = "model.mtp_layers.0.token_layernorm.weight"
                elif name == "mtp.pre_fc_norm_hidden.weight":
                    name = "model.mtp_layers.0.hidden_layernorm.weight"
                elif name == "mtp.norm.weight":
                    name = "model.mtp_layers.0.final_layernorm.weight"
                elif name.startswith("mtp.layers.0."):
                    name = "model.mtp_layers.0." + name[len("mtp.layers.0."):]
                else:
                    continue
