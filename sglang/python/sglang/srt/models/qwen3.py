...
class Qwen3ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            # The pushed FSDP state carries mtp.* (the draft head). This plain-
            # Qwen3 TARGET has no mtp.* params; without this skip the stacked
            # branch maps mtp.*.q_proj -> qkv_proj and KeyErrors. The draft
            # (Qwen3ForCausalLMMTP) already consumed mtp.* upstream.
            if "mtp" in name:
                continue
