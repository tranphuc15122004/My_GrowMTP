...
def unwrap_ipc_tensors(named_tensors, tp_rank, device):
    """Open each CUDA-IPC handle exactly once, then detach to real tensors.

    GrowMTP IPC-leak fix (sglang PR #17870): must be called once after
    deserialization so the same handle is not opened by multiple runners.
    """
    result = []
    for name, tensor in named_tensors:
        if isinstance(tensor, LocalSerializedTensor):
