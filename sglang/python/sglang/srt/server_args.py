...
class ServerArgs:
    ...
    growmtp_collect_signals: bool = False
    growmtp_teacher_topk: int = 64
    speculative_classic_rejection_sampling: bool = False
    ...
    def _handle_speculative_decoding(self):
        if self.growmtp_collect_signals or self.speculative_classic_rejection_sampling:
            if self.speculative_algorithm != "EAGLE" or self.speculative_eagle_topk != 1:
                raise ValueError("GrowMTP requires EAGLE with speculative_eagle_topk=1")
            self.disable_overlap_schedule = True
            if self.growmtp_collect_signals:
                self.disable_radix_cache = True
                if not self.speculative_classic_rejection_sampling:
                    raise ValueError("GrowMTP training records require classic rejection sampling")
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument("--growmtp-collect-signals", action="store_true")
        parser.add_argument("--growmtp-teacher-topk", type=int, default=64)
        parser.add_argument("--speculative-classic-rejection-sampling", action="store_true")
        parser.add_argument(
