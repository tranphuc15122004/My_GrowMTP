...
class SchedulerOutputProcessorMixin:
    ...
    def stream_output_generation(
        ...
        output_mtp_topk_val = []
        output_mtp_topk_idx = []
        output_mtp_draft_tokens = []
        output_mtp_hidden = []
        output_mtp_position = []
        output_mtp_accept_len = []
        output_mtp_accepted_hidden = []
        output_mtp_accepted_tokens = []
        output_mtp_prefill_hidden = []
        output_mtp_prefill_tokens = []
        output_mtp_prefill_position = []
        output_mtp_prompt_hidden = []
        output_mtp_prompt_ids = []
        ...
        for req in reqs:
            ...
            if should_output:
                ...
                mtp_off = req.send_output_mtp_offset
                mtp_end = len(req.output_mtp_topk_val)
                history_off = getattr(req, "send_output_mtp_history_offset", 0)
                history_end = len(req.output_mtp_accepted_hidden)
                output_mtp_topk_val.append(req.output_mtp_topk_val[mtp_off:mtp_end])
                output_mtp_topk_idx.append(req.output_mtp_topk_idx[mtp_off:mtp_end])
                output_mtp_draft_tokens.append(req.output_mtp_draft_tokens[mtp_off:mtp_end])
                output_mtp_hidden.append(req.output_mtp_hidden[mtp_off:mtp_end])
                output_mtp_position.append(req.output_mtp_position[mtp_off:mtp_end])
                output_mtp_accept_len.append(req.output_mtp_accept_len[mtp_off:mtp_end])
                output_mtp_accepted_hidden.append(req.output_mtp_accepted_hidden[history_off:history_end])
                output_mtp_accepted_tokens.append(req.output_mtp_accepted_tokens[history_off:history_end])
                prefill_off = req.send_output_mtp_prefill_offset
                prefill_end = len(req.output_mtp_prefill_hidden)
                if (len(req.output_mtp_prefill_tokens) != prefill_end
                        or len(req.output_mtp_prefill_position) != prefill_end):
                    raise ValueError(f"Re-prefill history is misaligned for request {req.rid}")
                output_mtp_prefill_hidden.append(req.output_mtp_prefill_hidden[prefill_off:prefill_end])
                output_mtp_prefill_tokens.append(req.output_mtp_prefill_tokens[prefill_off:prefill_end])
                output_mtp_prefill_position.append(req.output_mtp_prefill_position[prefill_off:prefill_end])
                req.send_output_mtp_prefill_offset = prefill_end
                output_mtp_prompt_hidden.append(req.output_mtp_prompt_hidden)
                output_mtp_prompt_ids.append(req.output_mtp_prompt_ids)
                req.send_output_mtp_offset = mtp_end
                req.send_output_mtp_history_offset = history_end
        ...
        if reqs or is_idle_batch:
            self.send_to_detokenizer.send_output(
                BatchTokenIDOutput(
                    ...
                    output_mtp_topk_val=output_mtp_topk_val,
                    output_mtp_topk_idx=output_mtp_topk_idx,
                    output_mtp_draft_tokens=output_mtp_draft_tokens,
                    output_mtp_hidden=output_mtp_hidden,
                    output_mtp_position=output_mtp_position,
                    output_mtp_accept_len=output_mtp_accept_len,
                    output_mtp_accepted_hidden=output_mtp_accepted_hidden,
                    output_mtp_accepted_tokens=output_mtp_accepted_tokens,
                    output_mtp_prefill_hidden=output_mtp_prefill_hidden,
                    output_mtp_prefill_tokens=output_mtp_prefill_tokens,
                    output_mtp_prefill_position=output_mtp_prefill_position,
                    output_mtp_prompt_hidden=output_mtp_prompt_hidden,
                    output_mtp_prompt_ids=output_mtp_prompt_ids,
