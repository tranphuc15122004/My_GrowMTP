...
class Req(ReqDllmMixin):
    ...
    def __init__(
        ...
        self.output_mtp_topk_val: list = []       # List[Tensor(k, topK)]
        self.output_mtp_topk_idx: list = []       # List[Tensor(k, topK)]
        self.output_mtp_draft_tokens: list = []   # List[Tensor(k+1,)]
        self.output_mtp_hidden: list = []         # List[Tensor(H,)]
        self.output_mtp_position: list = []       # List[int]
        self.output_mtp_accept_len: list = []       # List[int]
        self.output_mtp_accepted_hidden: list = []   # List[Tensor(n_acc, H)] — no-force-reject
        self.output_mtp_accepted_tokens: list = []   # List[Tensor(n_acc,)] — no-force-reject
        self.output_mtp_prefill_hidden: list = []
        self.output_mtp_prefill_tokens: list = []
        self.output_mtp_prefill_position: list = []
        self.send_output_mtp_prefill_offset: int = 0
        self._mtp_prefill_next_position: int = 0
        self.send_output_mtp_offset: int = 0
        self.output_mtp_prompt_hidden = None      # Tensor(prompt_len, H) - set once at prefill
        self.output_mtp_prompt_ids = None          # Tensor(prompt_len,) - set once at prefill
