...
class ReqState:
    ...
    mtp_topk_val: list = dataclasses.field(default_factory=list)
    mtp_topk_idx: list = dataclasses.field(default_factory=list)
    mtp_draft_tokens: list = dataclasses.field(default_factory=list)
    mtp_hidden: list = dataclasses.field(default_factory=list)
    mtp_position: list = dataclasses.field(default_factory=list)
    mtp_accept_len: list = dataclasses.field(default_factory=list)
    mtp_accepted_hidden: list = dataclasses.field(default_factory=list)
    mtp_accepted_tokens: list = dataclasses.field(default_factory=list)
    mtp_prefill_hidden: list = dataclasses.field(default_factory=list)
    mtp_prefill_tokens: list = dataclasses.field(default_factory=list)
    mtp_prefill_position: list = dataclasses.field(default_factory=list)
    mtp_prompt_hidden: object = None  # Tensor(prompt_len, H) — set once from prefill
    mtp_prompt_ids: object = None     # Tensor(prompt_len,) — set once from prefill
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            if getattr(recv_obj, "customized_info", None):
                for k, v in recv_obj.customized_info.items():
                    meta_info[k] = v[i]
            # Accumulate prompt hidden (one-shot, from first recv_obj that has it)
            if getattr(recv_obj, 'output_mtp_prompt_hidden', None) and recv_obj.output_mtp_prompt_hidden[i] is not None:
                if state.mtp_prompt_hidden is None:
                    state.mtp_prompt_hidden = recv_obj.output_mtp_prompt_hidden[i]
                    state.mtp_prompt_ids = recv_obj.output_mtp_prompt_ids[i]
            # Accumulate per-step decode signals
            if getattr(recv_obj, 'output_mtp_topk_val', None) and recv_obj.output_mtp_topk_val[i]:
                state.mtp_topk_val.extend(recv_obj.output_mtp_topk_val[i])
                state.mtp_topk_idx.extend(recv_obj.output_mtp_topk_idx[i])
                state.mtp_draft_tokens.extend(recv_obj.output_mtp_draft_tokens[i])
                state.mtp_hidden.extend(recv_obj.output_mtp_hidden[i])
                state.mtp_position.extend(recv_obj.output_mtp_position[i])
                state.mtp_accept_len.extend(recv_obj.output_mtp_accept_len[i])
            _acc_h = getattr(recv_obj, 'output_mtp_accepted_hidden', None)
            _acc_t = getattr(recv_obj, 'output_mtp_accepted_tokens', None)
            if _acc_h and _acc_h[i]:
                state.mtp_accepted_hidden.extend(_acc_h[i])
                state.mtp_accepted_tokens.extend(_acc_t[i])
            prefill_fields = ("prefill_hidden", "prefill_tokens", "prefill_position")
            prefill_blocks = [getattr(recv_obj, "output_mtp_" + name, None) for name in prefill_fields]
            if any(block is not None for block in prefill_blocks):
                if (any(block is None or i >= len(block) for block in prefill_blocks)
                        or len({len(block[i]) for block in prefill_blocks}) != 1):
                    raise ValueError("Streamed re-prefill history is misaligned")
                for name, block in zip(prefill_fields, prefill_blocks):
                    getattr(state, "mtp_" + name).extend(block[i])
            # NOTE: state.finished is set AFTER this code runs (line ordering),
            # so check recv_obj.finished_reasons[i] directly.
            if state.mtp_topk_val and recv_obj.finished_reasons[i] is not None:
                meta_info["mtp_signals"] = {
                    "topk_val": state.mtp_topk_val,
                    "topk_idx": state.mtp_topk_idx,
                    "draft_tokens": state.mtp_draft_tokens,
                    "hidden": state.mtp_hidden,
                    "position": state.mtp_position,
                    "accept_len": state.mtp_accept_len,
                    "accepted_hidden": state.mtp_accepted_hidden if state.mtp_accepted_hidden else None,
                    "accepted_tokens": state.mtp_accepted_tokens if state.mtp_accepted_tokens else None,
                    "prefill_hidden": state.mtp_prefill_hidden,
                    "prefill_tokens": state.mtp_prefill_tokens,
                    "prefill_position": state.mtp_prefill_position,
                    "prompt_hidden": state.mtp_prompt_hidden,
                    "prompt_ids": state.mtp_prompt_ids,
                }
