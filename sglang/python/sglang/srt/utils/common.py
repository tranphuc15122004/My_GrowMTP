...
def _wait_for_reap_or_raise(procs, wait_timeout: float) -> None:
    ...
    # An exited orphan can remain a zombie under a container PID 1. Its CUDA
    # context is already released; waiting for the caller to reap it cannot work.
    procs = [process for process in procs if running(process)]
    warn_at = min(10.0, wait_timeout / 2)
    gone, alive = psutil.wait_procs(procs, timeout=warn_at)
    alive = [process for process in alive if running(process)]
    if not alive:
        return
    logger.warning(
