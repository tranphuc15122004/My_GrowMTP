...
# When enabled, draft tokens are SAMPLED from q (set in eagle_worker.draft_forward),
# verify accepts w.p. min(1, p_target/p_draft), and on reject samples from the
# normalized residual (p_t - p_d)_+. Off => original target-only verify (unchanged).
...
class EagleVerifyInput(SpecInput, EagleVerifyInputV2Mixin):
    ...
    def verify(
        ...
        if is_all_greedy or not TREE_SPEC_KERNEL_AVAILABLE:
            ...
            # Gated on speculative_classic_rejection_sampling, topk==1 chain, and the draft having
            # surfaced its sampled probs (set by eagle_worker.draft_forward via the
            # cuda-graph-surfaced static buffers).
            if get_global_server_args().speculative_classic_rejection_sampling and (
                getattr(self, "_classic_rs_draft_probs", None) is None
                or getattr(self, "_classic_rs_full_draft_probs", None) is None
            ):
                raise RuntimeError("Classic rejection sampling requires the actual full draft distributions")
            if (
                get_global_server_args().speculative_classic_rejection_sampling
                and self.topk == 1
                and getattr(self, "_classic_rs_draft_probs", None) is not None
            ):
                from sglang.srt.speculative.acceptance import classic_rejection_sample
                draft_ids = candidates[:, 1:self.draft_token_num].long()
                num_correct, bonus = classic_rejection_sample(
                    target_probs, draft_ids, self._classic_rs_full_draft_probs)
                accept_index[:, 0] = self.retrieve_index[:, 0].int()
                for j in range(self.draft_token_num - 1):
                    accepted = num_correct > j
                    accept_index[accepted, j + 1] = self.retrieve_index[accepted, j + 1].int()
                    predict[self.retrieve_index[accepted, j].long()] = draft_ids[accepted, j].to(predict.dtype)
                bonus_indices = self.retrieve_index.gather(1, num_correct[:, None].long()).squeeze(1)
                predict[bonus_indices.long()] = bonus.to(predict.dtype)
                num_correct_drafts.copy_(num_correct)
        ...
        for i, (req, accept_index_row) in enumerate(zip(batch.reqs, accept_index_cpu)):
            if get_global_server_args().growmtp_collect_signals:
                # Record the actual rejection boundary before EOS/length truncation mutates it.
                req.output_mtp_accept_len.append(sum(idx >= 0 for idx in accept_index_row))
            num_accept_tokens = 0
...
class EagleDraftInput(SpecInput, EagleDraftInputV2Mixin):
    ...
    def filter_batch(self, new_indices: torch.Tensor, has_been_filtered: bool = True):
        ...
        _rs_fp = getattr(self, "_classic_rs_full_probs", None)
        if _rs_fp is not None:
            self._classic_rs_full_probs = (
                _rs_fp[: len(new_indices)] if has_been_filtered else _rs_fp[new_indices]
            )
    ...
    def merge_batch(self, spec_info: "EagleDraftInput"):
        ...
        if len(self.topk_index) == 0:
            ...
            # GrowMTP classic-RS: adopt other's draft_0 full dist when self is idle stub
            self._classic_rs_full_probs = getattr(
                spec_info, "_classic_rs_full_probs", None
            )
            return
        ...
        _rs_a = getattr(self, "_classic_rs_full_probs", None)
        _rs_b = getattr(spec_info, "_classic_rs_full_probs", None)
        if _rs_a is not None and _rs_b is not None:
            self._classic_rs_full_probs = torch.cat([_rs_a, _rs_b], dim=0)
        else:
            self._classic_rs_full_probs = None
